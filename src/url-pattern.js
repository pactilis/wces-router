...
const factory = function() {
  let P;
  let UrlPattern;
  let astNodeContainsSegmentsForProvidedParams;
  let astNodeToNames;
  let astNodeToRegexString;
  let baseAstNodeToRegexString;
  let concatMap;
  let defaultOptions;
  let escapeForRegex;
  let getParam;
  let keysAndValuesToObject;
  let newParser;
  let regexGroupCount;
  let stringConcatMap;
  let stringify;
  ...
  P.sequence = function() {
    let parsers;
    parsers = arguments.length >= 1 ? slice.call(arguments, 0) : [];
    return function(input) {
      let i;
      let length;
      let parser;
  ...
  P.pick = function() {
    let indexes;
    let parsers;
    (indexes = arguments[0]), (parsers = arguments.length >= 2 ? slice.call(arguments, 1) : []);
    return function(input) {
      let array;
      let result;
    ...
    if (this.isRegex) {
      ...
      if (arg2 != null) {
        ...
        if (arg2.length !== groupCount) {
          throw new Error(
            `regex contains ${groupCount} groups but array of group names contains ${arg2.length}`,
          );
        }
        this.names = arg2;
      }
...
export const UrlPattern = factory();